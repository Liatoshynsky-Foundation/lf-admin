import { mapToUpdateMediaMentionInput } from './media.mapper';
...
const buildSeoData = (): SeoData => ({
  ...
  ogImage: 'https://a.com/img.jpg'
});

describe('mapToUpdateMediaMentionInput', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  it('delegates mapping to mapSeoBase with the provided data', () => {
    ...
    mapToUpdateMediaMentionInput(data);
  ...
  it('returns exactly the spread output of mapSeoBase', () => {
    ...
    const fakeResult = {
      ...
      coverImage: { src: 'x', alt: { uk: '', en: '' } }
    };
    mockedMapSeoBase.mockReturnValue(fakeResult as never);

    const result = mapToUpdateMediaMentionInput(data);
