...
export const styles = {
  individualWorkRow: (gridTemplate: string, firstColWidth: string, offsetFirstCol: boolean): SxProps<Theme> => ({
    display: 'grid',
    gridTemplateColumns: offsetFirstCol
      ? `calc(${firstColWidth} + 32px)${gridTemplate.slice(firstColWidth.length)}`
      : gridTemplate,
    columnGap: tableGap,
    alignItems: 'stretch',
